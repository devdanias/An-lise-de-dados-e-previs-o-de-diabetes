...
1. F1 Score (média harmônica entre precisão e recall) - principal métrica devido ao desbalanceamento de classes
2. ROC AUC (capacidade de discriminação entre classes)
3. Precisão (importante para minimizar falsos positivos)
...
1. **Principais achados**:
   - Pacientes com pressão alta e colesterol alto têm probabilidade significativamente maior de desenvolver diabetes
   - O IMC é um forte indicador de risco para diabetes
   - A proporção de diabetes na população estudada é de X%

2. **Modelo preditivo**:
   - O melhor modelo foi o [Nome do Modelo] com F1 Score de X e AUC-ROC de Y
   - As variáveis mais importantes para a predição foram: [listar as top 3]

3. **Recomendações**:
   - Programas de prevenção deveriam focar em pacientes com pressão alta e colesterol elevado
   - Monitoramento mais frequente do IMC pode ajudar na identificação precoce de risco
   - O modelo desenvolvido pode ser integrado a sistemas de triagem para identificar pacientes de alto risco

**Limitações e trabalhos futuros**:
- O dataset possui limitações em [descrever limitações]
- Futuros trabalhos poderiam incorporar [sugestões de melhorias]
"""
